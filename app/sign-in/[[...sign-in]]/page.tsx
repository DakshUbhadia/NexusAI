...
function FeaturePill({
  ...
  return (
    ...
    >
      <motion.div
        className="flex size-9 shrink-0 items-center justify-center rounded-xl border border-[var(--accent-primary)]/20 bg-[var(--accent-primary)]/15 text-[var(--accent-primary)] backdrop-blur-sm"
      ...
      <div>
        <h3
          className="mb-0.5 text-[0.8rem] font-semibold tracking-wide text-[var(--text-primary)]"
          style={{ fontFamily: "'Bricolage Grotesque', sans-serif" }}
        >
          {title}
        </h3>
        <p className="text-[0.7rem] leading-relaxed text-[var(--text-secondary)]">{desc}</p>
      </div>
    </motion.div>
  )
}
...
export default function SignInPage() {
  ...
  /* ── Spline Imperative Load + Custom Element Registry Fix ────────── */
  useEffect(() => {
    // 1. Strict global check to prevent Turbopack 'Already Defined' Error
    if (typeof window !== "undefined") {
      // @ts-expect-error adding custom property to window for strict tracking
      if (!window.__SPLINE_SETUP_DONE__ && !customElements.get("spline-viewer")) {
        // @ts-expect-error adding custom property to window for strict tracking
        window.__SPLINE_SETUP_DONE__ = true;
    ...
    // 2. Continuous Badge Killer (Pierces Shadow DOM)
    const kill = () => {
    ...
    kill()
    const interval = setInterval(kill, 150)
  ...
  return (
    <main className="relative h-screen overflow-hidden bg-[var(--auth-bg-deep)] text-[var(--text-primary)] lg:grid lg:grid-cols-[minmax(0,13fr)_minmax(0,7fr)]">
      {/* ── Force sign-out any lingering Clerk session on mount ── */}
      <ForceSignOut />
        ...
        <div className="pointer-events-none absolute inset-0 z-20 flex flex-col px-10 py-8">
          ...
          >
            <div className="relative">
              <div className="absolute inset-0 scale-150 rounded-xl bg-[var(--accent-primary)]/30 blur-xl" />
              <Image
                unoptimized={true}
                src="/logo1.png"
                alt="Nexus"
          ...
          {/* Grouped Headline & Details (Bottom Left) */}
          <div className="max-w-2xl mb-8 flex flex-col gap-6">
            
            {/* Headline */}
            <h1
              className="font-black leading-[0.92] tracking-[-0.04em] text-[var(--text-primary)]"
              style={{
                fontFamily: "'Bricolage Grotesque', sans-serif",
            ...
            {/* Details & Feature Pills */}
            <div>
              <motion.p
                className="mb-2 max-w-lg text-[0.82rem] font-light leading-relaxed text-[var(--text-secondary)]"
          ...
          <motion.p
            className="shrink-0 text-[0.65rem] text-[var(--text-secondary)]"
            style={{ fontFamily: "'Bricolage Grotesque', sans-serif" }}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
      ...
      <section className="relative flex h-screen w-full flex-col items-center overflow-hidden lg:min-w-0 lg:bg-transparent">
        ...
        <div className="relative z-[10] flex w-full flex-1 flex-col items-center justify-start px-4 sm:px-6">
          ...
          >
            <div className="relative w-full overflow-hidden rounded-2xl bg-[var(--auth-card-bg)] px-5 py-5 backdrop-blur-2xl">
              ...
              <div className="pointer-events-none absolute inset-x-0 bottom-0 h-px bg-gradient-to-r from-transparent via-[var(--border-accent)]/30 to-transparent" />
