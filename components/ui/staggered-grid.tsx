...
export function StaggeredGrid({
    ...
    const mixedGridItems: (GridNode | 'BENTO_GROUP' | 'EMPTY')[] = Array.from({ length: 21 }, (_, i) => {
    ...
    return (
        ...
        >
            ...
            <section className="grid place-items-center w-full relative">
                <div ref={gridFullRef} className="grid--full relative w-full my-8 h-auto aspect-[1.8] max-w-5xl mx-auto p-2 grid gap-3 grid-cols-7 grid-rows-5">
                            ...
                            return (
                                <div key="bento-group" data-col={2} className="grid__item bento-container col-span-3 row-span-1 relative z-20 flex items-center justify-center gap-2 h-full w-full will-change-transform">
                                    {bentoItems.map((bentoItem, index) => {
                                        ...
                                        const CardWrapper = bentoItem.href ? 'a' : 'div';
                                        return (
                                            <CardWrapper
                                                key={bentoItem.id}
                                                href={bentoItem.href}
                                                target={bentoItem.href?.startsWith('http') ? '_blank' : undefined}
                                                rel={bentoItem.href?.startsWith('http') ? 'noopener noreferrer' : undefined}
                                                className={cn(
                                ...
                                <figure key={`img-${i}`} data-col={i % 7} className="grid__item m-0 relative z-10 [perspective:800px] will-change-[transform,opacity] group cursor-pointer">
                                    <div className="grid__item-img w-full h-full [backface-visibility:hidden] will-change-transform rounded-xl overflow-hidden shadow-sm border border-zinc-200 dark:border-zinc-900 bg-zinc-100 dark:bg-zinc-950 flex flex-col items-center justify-center transition-all duration-500 ease-out group-hover:scale-105 group-hover:shadow-xl group-hover:border-transparent">
                                        ...
                                        <div className="relative z-10 flex flex-col items-center justify-center gap-1.5 p-2">
                                            {/* Icon */}
                                            <div className="w-6 h-6 text-zinc-400 dark:text-zinc-500 transition-all duration-300 group-hover:text-white group-hover:scale-110 flex items-center justify-center">
                                                {item.icon}
                                            </div>
                                            ...
                                            <div className="text-center opacity-0 group-hover:opacity-100 transform translate-y-1 group-hover:translate-y-0 transition-all duration-300 delay-75">
                                                <span className="block text-[8px] font-medium text-white/90 uppercase tracking-wider mb-0.5">Built with</span>
                                                <span className="block text-xs font-bold text-white tracking-tight leading-tight">{item.label}</span>
                                            </div>
                                        </div>
                                    </div>
                                </figure>
